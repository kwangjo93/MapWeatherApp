...
extension PresentingForecast {
    func toWeatherDays() -> WeatherOfDays {
        ...
        var temperaturesByDay: [String: (min: Double, max: Double)] = [:]
        ...
        for forecast in forcasts {
            ...
            if let temperatures = temperaturesByDay[day] {
                ...
                temperaturesByDay[day] = (min: minTemp, max: maxTemp)
            } else {
                temperaturesByDay[day] = (min: forecast.tempMin, max: forecast.tempMax)
            }
        }
        ...
        for day in sortedDays {
            ...
            let weatherList = WeatherLists(day: day, tempMin: temperatures.min, tempMax: temperatures.max, imageUrl: imageUrl)
